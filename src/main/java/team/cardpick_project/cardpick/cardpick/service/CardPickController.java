...
public class CardPickController {
    ...
    public List<CardResponse> getCardsByConditions(@RequestParam(required = false) String issuer, @RequestParam(required = false) List<String> categories){
        return cardPickService.getCardsByConditions(issuer, categories);
    }
    //현재 선택된 조건에 맞는 카드 개수
    @GetMapping("/conditions/count")
    public Long getCountByConditions(@RequestParam(required = false) String issuer, @RequestParam List<String> categories){
